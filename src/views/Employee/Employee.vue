<template>
    ...
    <div class="p-3">
      <div class="flex justify-between items-center">
          ...
          <h2 class="uppercase dark:text-white text-gray-900 text-2xl font-bold">Сотрудники</h2>
              ...
              <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                  ...
                  <form >
                      <div class="grid gap-4 mb-4 sm:grid-cols-1">
                          <div>
                              <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ф.И.О.Cотрудника</label>
                              <input v-model="contactInfo.name" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Введите свое полное имя!" required="">
                          </div>
                          <div>
                              <label for="profession" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Позиция</label>
                              <input v-model="contactInfo.profession" type="text" name="profession" id="profession" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Занимаемая должность" required="">
                          </div>
                          <div>
                            <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Информация о сотруднике</label>
                            <input v-model="contactInfo.description" type="text" name="description" id="description" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Информация о сотруднике" required="">
                        </div>
                        <div class="">
              ...
              <div class="overflow-x-auto">
                  <table class="w-full text-sm">
                      <thead class="text-md text-white font-bold uppercase bg-gray-500 ">
                          <tr class="">
                              <th scope="col" class="px-4 py-3 text-left">Ф.И.О.</th>
                              <th scope="col" class="px-4 py-3 text-left">Позиция</th>
                              <th scope="col" class="px-4 py-3 text-left">Информация о сотруднике</th>
                              <th scope="col" class="px-4 py-3 text-left">Фото сотрудника</th>
                      ...
                      <tbody class="">
                          <tr v-for="el in computedList" :key="el.id" class="border-b dark:border-gray-700">
                              ...
                              <td class="px-4 py-3">{{el.profession}}</td>
                              <td class="px-4 py-3">{{el.description}}</td>
                              <td class="px-4 py-3"><img :src="el.mediaUrl" alt="Icon" class="w-[50px]"></td>
  ...
  const addContact=(evet)=>{
      ...
      employees.create(contact).then((res)=>{
          if(res.status == 201){
              contactInfo.name=''
              contactInfo.profession=''
              contactInfo.description=''
  ...
  const modifyContact=(event)=>{
      ...
      employees.update(id, contact).then((res)=>{
          if(res.status == 200){
  ...
  const updateContact = (id)=>{
      ...
      contactInfo.profession = foundContact[0].profession
